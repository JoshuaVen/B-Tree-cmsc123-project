...
public class BTreeMain1<T extends Comparable<T>> {
    ...
    /**
     * Returns all the element of the tree in level-order traversing.
     */
    ...
    public String levelOrder() {
      ...
      lvlOrder = new LinkedList<Node>();
      lvlOrder.add(root);
      while (true) {
        int nodeCount = lvlOrder.size();
        if (nodeCount == 0) {
          break;
        }
        while (nodeCount > 0 ) {
          Node node = lvlOrder.peek();
